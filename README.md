...
| [02](./02-操作系统与底层/) | 操作系统与底层 | 内核、内存管理、进程调度、文件系统、崩溃诊断 | 🌱 起步（Windows 崩溃诊断 1 篇） |
| [03](./03-编程与开发/) | 编程与开发 | 语言特性、工程实践、架构设计、Web 前后端通信、云开发 CloudBase、C# 语言核心、Git 版本控制 | 🌱 起步（Web 前后端通信 + 云开发 CloudBase + C# 委托与 Action + Git 版本控制，4 篇） |
...
| [08](./08-经验-整理/) | 经验文档 | 工作与开发经验随笔（.docx 原文） | 🌱 起步（33 篇） |
...
- [04 · CAN 总线协议详解](./01-计算机硬件/总线/04-CAN总线协议详解.md) — 闭环/开环拓扑与差分电平、五种帧类型、数据帧 7 段拆解、非破坏性仲裁四法则、位时序与同步、错误三态机、报文解析实战（DBC/工具链）、CAN FD 与十大高频坑
...
- [04 · USB 3.x 接口与 U 盘详解](./01-计算机硬件/插槽与接口/04-USB3x接口与U盘详解.md) — USB 3.x 命名乱象(Gen1/2/2×2)、xHCI 控制器、接口颜色编码、编码开销与真实速率、U 盘性能瓶颈、UASP/文件系统、USB PD、USB4/雷电，工业多相机根集线器带宽避坑
- [05 · USB4 与雷电 4 一线连详解](./01-计算机硬件/插槽与接口/05-USB4与雷电4一线连详解.md) — USB4=雷电3开放版、强制 Type-C、隧道化(PCIe+DP+USB3)、一线连数据+显示+供电、带宽分配、Alt Mode、认证标识、线材长度、eGPU/NVMe/扩展坞实战，与 USB 3.x 及机器视觉呼应
...
- [机械硬盘故障排查指南](./01-计算机硬件/综合/机械硬盘故障排查指南.md) — SATA 机械盘坏道/间歇性掉盘的硬件+S.M.A.R.T 排查，明确严禁 `chkdsk /r`
...
### 机器视觉硬件（工业相机 / 成像采集）
- [01 · 工业相机详解](./07-算法/机器视觉硬件/01-工业相机详解.md) — 触发（内/外/软/硬）·帧率·曝光·亮度·增益全解，附分类/接口/选型/误区/术语速查，并打通 HALCON `open_framegrabber` 采集对接
- [02 · 工业相机曝光增益帧率与参数下发详解](./07-算法/机器视觉硬件/02-工业相机曝光增益帧率与参数下发详解.md) — 上篇扩展：增益=ISO 与噪点机制、曝光拖影公式、帧率=曝光+读出两 regime、ROI 破帧率上限、参数下发耗时与"假帧率"陷阱（结合海康 SDK 实战）
- [03 · 镜头光源选型详解](./07-算法/机器视觉硬件/03-镜头光源选型详解.md) — 成像三件套收官（镜+光）：镜头焦距/FOV/WD/靶面匹配/分辨率/景深/畸变/远心选型公式与决策表；光源七大类型/明场暗场/互补色/偏振/频闪；三件套协同精度计算与 7 步选型流程
- [04 · 工业与安防相机局域网网络配置详解](./07-算法/机器视觉硬件/04-工业与安防相机局域网网络配置详解.md) — 把相机接上网：纯内网 IP/掩码/网关规划、/24 vs /16、Win/macOS 配置、Ping 诊断与排查 SOP；重点扩展网卡"属性-高级"——巨型帧/接收缓存帧/传送缓冲区/环保节能/节能以太网(EEE)的作用·配置·影响·建议值，附最佳实践速查表

---

### Web 前后端通信（HTML/JS 与服务器数据交互）
- [01 · HTML 前后端通信详解](./03-编程与开发/Web前后端通信/01-HTML前后端通信详解.md) — 从 jQuery `$.post` 与 `$.ajax` 两段真实代码对比切入，覆盖 AJAX / Fetch / Axios / WebSocket / SSE 全栈通信技术、HTTP 方法 / 状态码 / Content-Type、跨域 CORS、认证安全、前后端分离架构与现代重写方案

---

### 云开发 CloudBase（腾讯云 Serverless 后端）
- [01 · CloudBase 开发部署详解](./03-编程与开发/云开发CloudBase/01-CloudBase开发部署详解.md) — 源自本地店 AI 自动化方案，详解云函数 / 云数据库 / 云存储 / 定时触发器 / 云接入 / 微信云调用，含完整 CLI 部署流水线与三大服务 CloudBase 落地映射

---

### C# 语言核心（委托 / 事件 / Action / Func）
- [01 · 委托（Delegate）与 `public static Action` 详解](./03-编程与开发/CSharp/01-CSharp委托与Action详解.md) — 讲清委托本质（类型安全的函数引用 / MulticastDelegate 继承链），内置委托家族 Action·Func·Predicate 的选用，`public static Action` 全局钩子语义，多播 / 协变逆变 / 闭包 / 事件 / 异步高级用法，八大使用场景、十大坑与选型决策表

---

### Git 版本控制（clone / commit / 分支 / 合并 / 变基）
- [01 · Git 版本控制详解](./03-编程与开发/Git版本控制/01-Git版本控制详解.md) — 以"不可变快照链 + 可移动指针 + 三区数据流"心智模型详解 clone / branch / checkout / add / commit / pull / push / merge / rebase / stash / tag 十一大命令，配 9 张 matplotlib 可视化图（三区数据流 / 快照链 / 分支互不干扰 / 三方合并 / rebase 前后 / stash 栈 / tag 里程碑 / pull 拆解），附速查表、冲突 SOP、reflog 后悔药、GUI 工具推荐、工作流选型与十大高频坑
