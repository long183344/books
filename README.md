...
- [章节总结 · 第 27 章 Tuple（元组）· 中卷](./07-算法/HALCON/章节总结/27-Tuple(中).md) — **元组数据处理车间 7 子族 47 算子**（Conversion 数据转换 12 + Creation 元组构造 5 + Data Containers 字典 10 + Element Order 元素序 2 + Features 统计特征 11 + Logical Operations 逻辑运算 4 + Manipulation 插入/删除/替换 3），「从字符串格式化到字典封装，从统计提炼到增删改——元组从裸数据到可交付产品的全流程车间」——Conversion：`tuple_string`(C printf 格式 ⚠️ 不是 C# `{0:F3}`)+`tuple_number`（失败抛错不像 Python float 返回 NaN）+`tuple_round`(二进制舍入 ⚠️ 0.005→0.00，要 Bankers 用 `tuple_real(tuple_int(T+0.5*sgn))`)+`tuple_chr/chrt/ord/ords`（ASCII 互转 ⚠️ 负数抛错）+`tuple_int/real`+`handle_to_integer/integer_to_handle`（句柄 ↔ int 反射）；Creation：`tuple_gen_const`(N 个相同元素)+`tuple_gen_sequence`(⚠️ 含 End，从 Start 加 Step 直到越过)+`tuple_concat`+`tuple_rand`(设负 Seed 才能复现)+`clear_handle`；Containers（HALCON 内置字典）：`create_dict`+`set/get_dict_tuple/object`(⚠️ Key 含小数点会被嵌套访问：`a.b` = dict['a']['b'])+`read/write_dict`(.hdict 私有协议 ≠ 通用 JSON，跨语言用 `serialize_dict`)+`copy_dict`+`remove_dict_key`+`get_dict_param`；Element Order：`tuple_sort`(⚠️ 字典序 'Z'<'a'，混合类型抛错)+`tuple_inverse`(immutable 不改入参，区别 Python list.reverse())；Features：`tuple_min/max/sum/mean/median/deviation`(⚠️ 默认总体 σ 除 N，统计教材用样本 σ 除 N-1)+`tuple_length`+`tuple_histo_range`(⚠️ 桶不包含 Max，左闭右开)+`get_handle_object/param/tuple`；Logical Operations：`tuple_and/or/xor/not`(⚠️ 仅整数，bool 算 0/1，区别 NumPy 自动转)；Manipulation：`tuple_insert`(Index 超界**夹紧**不抛错)+`tuple_remove`(半开区间 [I1,I2)，Index1==I2 返回原)+`tuple_replace`(长度可变，等同 remove+insert)；上卷 63 + 中卷 47 = 110 算子已收官（附七瓣辐射美学思维导图 PNG，7 子族色谱对应），下卷 Selection(11) + Sets(4) + StringOperations(15) + Type(14) = 44 算子
