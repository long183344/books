...
- [章节总结 · 第 27 章 Tuple（元组）· 上卷](./07-算法/HALCON/章节总结/27-Tuple(上).md) — **HALCON 元组数值算子基座 3 子族 63 算子**（Arithmetic 算术运算 45 + Bit Operations 位运算 6 + Comparison 比较运算 12），「所有图像/区域/XLD 算子的中间数据载体——算术+位运算+比较三大数值基座」——四则+取模+符号（`tuple_add/mult/sub/div/mod/neg/sgn`）+ 取整（`tuple_floor/ceil`，注意 C++ trunc 语义差异）+ 三角+双曲（`tuple_sin/cos/tan/asin/acos/atan`/`atan2`⚠️Y/X 顺序，与 C/C++ 相反)+指数对数（`tuple_exp/exp10/exp2/log/log10/log2`，⚠️负数抛异常）+ 度↔弧度（`tuple_deg/rad`）+ 误差函数（`tuple_erf/erfc/lgamma/tgamma`，⚠️tgamma 极点返回 ±∞ 不抛异常）+ 数值稳定（`tuple_hypot` 比手写 `sqrt(a²+b²)` 抗溢出）+ 累积和（`tuple_cumul` 等价 numpy.cumsum）+ 浮点硬件（`tuple_ldexp`+`tuple_fmod`，区别 mod）+ 位运算（`tuple_band/bor/bxor/bnot/lsh/rsh`，⚠️仅整数+逻辑右移+回卷不抛异常）+ 比较分两类：非 elem 版返回标量（`tuple_equal/greater/less/...`，字典序），elem 版返回同长度 0/1 元组（`tuple_equal_elem/greater_elem/...`，配合 `tuple_select_mask` 做阈值筛选）；3 约定：长度不等广播 / bool 输出=0,1 / elem 后缀=逐元素；下卷 = Containers(10) + Conversion(12) + Creation(5) + ElementOrder(2) + Features(11) + LogicalOperations(4) + Manipulation(3) + Selection(11) + Sets(4) + StringOperations(15) + Type(14) = 91 算子（附三角辐射美学思维导图 PNG，Arithmetic 橙/Bit 绿/Comparison 紫）
