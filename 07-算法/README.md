...
| [章节总结/25-Tools(下)](./HALCON/章节总结/25-Tools(下).md) | HALCON 官方手册第 25 章 Tools **下卷 29 算子**（5 子族：Grid Rectification 网格校正 5 + Hough 霍夫变换 7 + Interpolation 散点插值 5 + Lines 直线属性 2 + Mosaicking 图像拼接 10），「HALCON 图像域几何增强与拼接——从棋盘格拉正到 360° 全景合成」——`create_rectification_grid`+`find_rectification_grid`+`connect_grid_points`+`gen_grid_rectification_map`（4 步网格校正流水线，把歪斜网格拉正为正视图） + `hough_line_trans`+`hough_lines`（标准 ρ-θ 累加 + 峰值提取，抗噪强于直接拟合）+ `hough_line_trans_dir`/`hough_lines_dir`（带边缘方向加权减少假峰） + `create_scattered_data_interpolator`+`interpolate_scattered_data_image`（RBF/薄板样条散点建连续函数并栅格化成图） + `line_orientation`（带符号 -π/2~π/2 方向角） + `proj_match_points_ransac`+`gen_projective_mosaic`（RANSAC 鲁棒配准 + 投影融合）+ `gen_spherical_mosaic`+`gen_cube_map_mosaic`+`gen_bundle_adjusted_mosaic`（球面/立方贴图/光束法平差全景）；全章 8 子族 103 算子全部收官（上 32 + 中 42 + 下 29）（附五角辐射美学思维导图，左上 Mosaic 大卡 10 算子全景拼接） |
